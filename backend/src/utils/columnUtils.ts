 ...
 */
export async function getNextColumnOrder(projectId: string): Promise<number> {
 ...
 */
export async function isOrderAvailable(projectId: string, order: number, excludeColumnId?: string): Promise<boolean> {
	const query: any = { project: projectId, order };
	if (excludeColumnId) {
 ...
 */
export async function reorderColumns(projectId: string): Promise<void> {
 ...
 */
export async function insertColumnAtOrder(
	projectId: string, 
	columnId: string, 
	targetOrder: number
): Promise<void> {
